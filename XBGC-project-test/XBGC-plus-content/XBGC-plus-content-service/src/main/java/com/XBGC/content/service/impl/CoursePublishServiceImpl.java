...
import com.XBGC.base.exception.CommonError;
...
import com.XBGC.content.mapper.CoursePublishMapper;
import com.XBGC.content.mapper.CoursePublishPreMapper;
...
import com.XBGC.content.model.po.CoursePublishPre;
import com.XBGC.content.service.CourseBaseInfoService;
import com.XBGC.content.service.CoursePublishService;
...
public class CoursePublishServiceImpl implements CoursePublishService {
    ...
    public void commitAudit(Long companyId, Long courseId) {
        ...
        if (auditStatus.equals("202003")) {
            XBGCPlusException.cast("课程已提交请等待审核");
        }
        //本机构只能提交本机构的课程
        //todo:本机构只能提交本机构的课程
        ...
        if (StringUtils.isEmpty(pic)) {
            XBGCPlusException.cast("请求上传课程图片");
        }
        //查询课程计划
        //课程计划信息
        List<TeachplanDto> teachplanTree = teachplanService.findTeachplanTree(courseId);
        if (teachplanTree == null || teachplanTree.size() == 0) {
            XBGCPlusException.cast("请编写课程计划");
        }
    ...
    public void publish(Long companyId, Long courseId) {
        ...
        if (coursePublishPre == null) {
            XBGCPlusException.cast("课程没有审核记录，无法发布");
        }
        //状态
        String status = coursePublishPre.getStatus();
        //课程如果没有审核通过不允许发布
        if (!status.equals("202004")) {
            XBGCPlusException.cast("课程没有审核通过不允许发布");
        }
...
//            configuration.setDirectoryForTemplateLoading(new File(classpath+"/templates/"));
    ...
    public void uploadCourseHtml(Long courseId, File file) {
        try {
            ...
            if (upload == null) {
                ...
                XBGCPlusException.cast("上传静态文件过程中存在异常");
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            XBGCPlusException.cast("上传静态文件过程中存在异常");
    ...
    private void saveCoursePublishMessage(Long courseId) {
        ...
        if (mqMessage == null) {
            XBGCPlusException.cast(CommonError.UNKOWN_ERROR);
        }
