package com.XBGC.media.service.impl;
...
import com.XBGC.media.mapper.MediaFilesMapper;
import com.XBGC.media.mapper.MediaProcessMapper;
import com.XBGC.media.model.dto.QueryMediaParamsDto;
import com.XBGC.media.model.dto.UploadFileParamsDto;
import com.XBGC.media.model.dto.UploadFileResultDto;
import com.XBGC.media.model.po.MediaFiles;
import com.XBGC.media.model.po.MediaProcess;
import com.XBGC.media.service.MediaFileService;
import io.minio.*;
...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public UploadFileResultDto uploadFile(Long companyId, UploadFileParamsDto uploadFileParamsDto, String localFilePath,String objectName) {
        ...
        if(!result){
            XBGCPlusException.cast("上传文件失败");
        }
        //入库文件信息
        MediaFiles mediaFiles = currentProxy.addMediaFilesToDb(companyId, fileMd5, uploadFileParamsDto, bucket_mediafiles, objectName);
        if(mediaFiles==null){
            XBGCPlusException.cast("文件上传后保存信息失败");
        }
        //准备返回的对象
     ...
     * @return com.XBGC.media.model.po.MediaFiles
     * @author Mr.w
     * @date 2025/2/12 21:22
     */
    @Transactional
    ...
    private void clearChunkFiles(String chunkFileFolderPath, int chunkTotal) {
        try {
            ...
            // 强制迭代结果确保删除完成
            minioClient.removeObjects(removeArgs).forEach(r -> {
                try {
                    r.get();
                } catch (Exception e) {
                    log.error("分块删除失败: {}", e.getMessage());
                }
            });
        } catch (Exception e) {
            log.error("分块文件清理异常: {}", e.getMessage());
        }
    }
  /*  private void clearChunkFiles(String chunkFileFolderPath,int chunkTotal){
